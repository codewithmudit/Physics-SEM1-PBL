...
/* Convert wavelength (meters) to Angstroms */
double to_angstrom(double lambda_m) {
    return lambda_m * 1e10; /* 1 m = 1e10 Angstrom */
}
...
int main(void) {
    ...
    while (1) {
        ...
        if (choice == 1) {
            ...
            if (scanf("%lf", &Dn) != 1) { clear_stdin(); continue; }

            printf("Enter diameter Dn+p (mm): ");
            if (scanf("%lf", &Dnp) != 1) { clear_stdin(); continue; }

            printf("Enter order n: ");
            if (scanf("%d", &n) != 1) { clear_stdin(); continue; }

            printf("Enter p (>0): ");
            if (scanf("%d", &p) != 1 || p <= 0) { clear_stdin(); continue; }

            printf("Enter radius of curvature R (mm): ");
            if (scanf("%lf", &R) != 1 || R <= 0) { clear_stdin(); continue; }

            printf("Enter refractive index mu: ");
            if (scanf("%lf", &mu) != 1 || mu <= 0) { clear_stdin(); continue; }
        ...
        /* ------------------------------------------------------------- */
            ...
            if (scanf("%d", &wchoice) != 1) { clear_stdin(); continue; }
            ...
            double R; int n;
            printf("Enter lens R (mm): "); scanf("%lf", &R);
            printf("Enter ring order n: "); scanf("%d", &n);
        ...
        /* ------------------------------------------------------------- */
            ...
            if (wchoice == 1) {
                printf("Enter lambda_vacuum (nm): "); scanf("%lf", &lambda_vac);
                printf("Enter mu: "); scanf("%lf", &mu);
                lambda_med = (lambda_vac * 1e-9)/mu;
            } else {
                printf("Enter lambda_medium (nm): "); scanf("%lf", &lambda_med);
                lambda_med *= 1e-9;
                mu = -1;
            }

            double R; int n;
            printf("Enter R (mm): "); scanf("%lf", &R);
